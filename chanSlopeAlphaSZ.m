function [slopeValsLog, slopeValsRel, alphaPeakLog, alphaPeakRel] = chanSlopeAlphaSZ(out, frex)
%input: 
...
if isfield(out, 'hjkh')
    ...
    allPow = squeeze(mean(out.power, 3)); 
    logPow = log10(allPow) ; 
    relPow = allPow ./ sum(allPow,2);
    slopeValsLog = zeros(1,2);
    slopeValsRel = zeros(1,2);
    alphaPeakLog = zeros(1,1);
    alphaPeakRel = zeros(1,1);
    ...
    gausMod = @(b,x) b(1)^2*exp(-( (x-b(2)).^2 ./ (2*b(3)^2) ) );
    beta0 = [1,10,2]; 

    detrendDat = logPow(frex>6 & frex<14) -...
        modelfun(mdl.Coefficients.Estimate, frex(frex>6 & frex<14)); 
    detrendDat = detrendDat - min(detrendDat); 
