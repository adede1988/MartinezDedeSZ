...
library(pracma)
library(glmtoolbox)
source('G:\\My Drive\\GitHub/MartinezDedeSZ/helperFuncsFinal.R')
...
repeats = 1000
savePath = 'G:\\My Drive\\GitHub\\MartinezDedeSZ\\bootstrapAnalysis\\'

#what's the stability of the large effect sizes for diagnosis? 
if(!file.exists(paste(savePath, "para_stab.csv", sep = ''))){
test <- results %>% filter(para > .035)
...
lookup <- paste(test$dependentVariable, test$eyeCon, test$diagGroup)


  for(ii in 1:repeats){
    print(ii)
    tmpDat = randomSplit(dat)
    tmpRes1 <- getResults(tmpDat[[1]], unique(test$dependentVariable), eyeCons, diagGroups)
    tmpRes1 <- tmpRes1[[1]]
    screen = paste(tmpRes1$dependentVariable, tmpRes1$eyeCon, tmpRes1$diagGroup)
    tmpRes1 <- tmpRes1[screen %in% lookup, ]
    tmpRes2 <- getResults(tmpDat[[2]], unique(test$dependentVariable), eyeCons, diagGroups)
    tmpRes2 <- tmpRes2[[1]]
    screen = paste(tmpRes2$dependentVariable, tmpRes2$eyeCon, tmpRes2$diagGroup)
    tmpRes2 <- tmpRes2[screen %in% lookup, ]
    
    stabilityMat[ii,] <- tmpRes1$para>.035 & tmpRes2$para>.035
  
  }
  stabilityMat_Para <- stabilityMat
  write.csv(stabilityMat_Para,
            paste(savePath, "para_stab.csv", sep = ''))
} else {
  stabilityMat <- read.csv(paste(savePath, "para_stab.csv", sep = ''))
...
results$Para_stab <- NA
results$Para_stab[which(results$para>.035)] <- colMeans(stabilityMat_Para)
...
results %>% 
  summarize(Diag = sum(Diag>.035) , 
            ...
            Diag_Age_Sex = sum(Diag_Age_Sex>.035 ) ) -> temp
